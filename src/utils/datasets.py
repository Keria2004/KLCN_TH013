...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb',
        ...
        elif self.dataset_name == 'datasets':
    ...
    def get_data(self):
        if self.dataset_name == 'imdb':
            ...
            ground_truth_data = self._load_folder_dataset()
        ...
        for class_name in os.listdir(folder):
            ...
            for filename in os.listdir(class_path):
                if filename.lower().endswith(('.jpg', '.png')):
                    ...
                    faces.append(img)
                    labels.append(class_to_arg[class_name])
        faces = np.expand_dims(np.array(faces, dtype='float32'), -1)
        labels = pd.get_dummies(labels).values
        return faces, labels

    train_faces, train_labels = load_images_from_folder(train_folder)
    test_faces, test_labels   = load_images_from_folder(test_folder)
